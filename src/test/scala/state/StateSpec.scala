...
class StateSpec extends FlatSpec {
  ...
  it should "sequence the state" in {
    import State._
    def constant(value: Int) = State[Int, Int](s => (value, value))
    val (list, _) = sequence(List(constant(3), constant(5))).run(1)
    list shouldBe List(3, 5)
  }
